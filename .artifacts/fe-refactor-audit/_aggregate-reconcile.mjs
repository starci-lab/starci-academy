...
for (const w of workers) {
  const j = JSON.parse(fs.readFileSync(path.join(ART, `2026-08-08-reconcile-worker-${w}.json`), "utf8"))
...
fs.writeFileSync(path.join(ART, "_reconcile-changed-files.txt"), files.join("\n"))
...
fs.writeFileSync(path.join(ART, "2026-08-08-reconcile-twin-parity.json"), JSON.stringify(summary.twinParity, null, 2))
fs.writeFileSync(path.join(ART, "2026-08-08-reconcile-status.json"), JSON.stringify(summary, null, 2))
...
if (twinIssues.length) console.log(JSON.stringify(twinIssues, null, 2))
