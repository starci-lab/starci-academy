...
const dirs = [
    "src/components/pages/ProfileOverviewPage",
    "src/components/pages/ProfileSkillsPage",
    "src/components/pages/ProfileRedirectPage",
    "src/components/pages/CvEditorPage",
    "src/components/pages/ProfileActivityPage",
    "src/components/pages/ProfileProjectsPage",
    ...
    "src/components/pages/EditProfilePage",
    "src/components/pages/ProfileCodingProblemPage",
    "src/components/pages/CvGalleryPage",
    "src/components/pages/ProfilePublicCvPage",
]
    ...
    for (const file of walk(path.join(ROOT, dir))) {
        ...
        for (const { name, tag, line, index } of frameTags(src)) {
            ...
            if (tokens && tokens.length !== 1) {
                rows.push({ rel, line, frame: name, gap: spacingToken(tag), cls: "multi-token", action: "split to one principle", snip: tag.replace(/\s+/g, " ").slice(0, 100) })
                continue
            }
            if (tokens) continue
            ...
            rows.push({ rel, line, frame: name, gap: spacingToken(tag), cls, action, snip: tag.replace(/\s+/g, " ").slice(0, 100) })
        }
        src.split("\n").forEach((l, i) => {
            if (/^\s*(\*|\/\/|\/\*)/.test(l)) return
            ...
            if (/\b(?:data-)?principle\b/.test(l)) {
                ...
                if (tokens && tokens.length !== 1) {
                    rows.push({ rel, line: i + 1, frame: "raw", gap: cm[1].match(SPACING)[0], cls: "multi-token", action: "fix singular", snip: l.trim().slice(0, 100) })
                }
                return
            }
            ...
            rows.push({ rel, line: i + 1, frame: "raw", gap: cm[1].match(SPACING)[0], cls, action, snip: l.trim().slice(0, 120) })
        })
    }
}
...
const byDir = {}
for (const r of rows) {
    const d = r.rel.split("/").slice(0, 4).join("/")
    byDir[d] = (byDir[d] ?? 0) + 1
}
...
fs.writeFileSync(
    path.join(ROOT, ".artifacts/fe-refactor-audit/_profile-holes.json"),
