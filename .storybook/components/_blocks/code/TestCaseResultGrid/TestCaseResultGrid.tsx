...
/**
 * ─────────────────────────────────────────────────────────────────────────────
 * STORYBOOK-LOCAL DESIGN SPEC — full port of `@/components/blocks/code/TestCaseResultGrid`.
 * Authored in Storybook (not `src`); synced to `src` later.
 * Depends on the sibling local {@link IOExampleCard} port (not `@/components`).
...
export interface TestCaseResultGridProps {
    ...
    className?: string
    /**
     * Dev/spec: tag this grid's own direct parts (the pill selector row + the
     * selected case's detail pane) so a BlockAnatomy panel can badge them.
     */
    showAnatomy?: boolean
}
 ...
 */
export const TestCaseResultGrid = ({ cases, labels, className, showAnatomy = false }: TestCaseResultGridProps) => {
    ...
    return (
        <div className={cn("flex flex-col gap-3", className)}>
            <div className="flex flex-wrap gap-2" data-anat-part={showAnatomy ? "Div.Pills" : undefined}>
            ...
            {active.isSample ? (
                <div data-anat-part={showAnatomy ? "IOExampleCard" : undefined}>
                    <IOExampleCard
                        rows={[
                            ...(active.input != null
                ...
                <p className="text-xs text-muted" data-anat-part={showAnatomy ? "Typography.HiddenNote" : undefined}>{labels.hidden}</p>
            )}
        </div>
    )
}
