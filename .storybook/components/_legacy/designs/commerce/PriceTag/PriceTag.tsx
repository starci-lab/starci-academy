    ...
    const breakdownContent = hasSaving ? (
        <div className="flex flex-col gap-1 p-3">
            ...
            <div className="flex items-center justify-between gap-3">
                <TypographyAtom.Base size="sm" text="Giá gốc" />
                <TypographyAtom.Base size="sm" text={formatPrice(original ?? discounted, currency)} />
            </div>
            {breakdown && original != null && original > breakdown.phase ? (
                <div className="flex items-center justify-between gap-3">
                    <TypographyAtom.Base size="sm"
                        color="muted"
                        text={breakdown.phaseLabel ? `Giai đoạn ${breakdown.phaseLabel}` : "Ưu đãi giai đoạn"}
                    />
                    <TypographyAtom.Base size="sm"
            ...
            {breakdown && breakdown.loyaltyPercent > 0 && breakdown.phase > discounted ? (
                <div className="flex items-center justify-between gap-3">
                    <TypographyAtom.Base size="sm"
                        ...
                        text={breakdown.loyaltyNote ? `Ưu đãi thành viên · ${breakdown.loyaltyNote}` : "Ưu đãi thành viên"}
                    />
                    <TypographyAtom.Base size="sm"
            ...
            <div className="flex items-center justify-between gap-3 border-t border-default pt-1">
                <TypographyAtom.Base size="sm" weight="medium" text="Bạn trả" />
                <TypographyAtom.Base size="sm" weight="medium" text={formatPrice(discounted, currency)} />
            </div>
        </div>
    ) : null
        ...
        <div className={cn("flex flex-col gap-1", className)} data-anat-part={anatPart}>
            ...
            {showSavingLine && hasSaving ? (
                <TypographyAtom.Base size="xs"
                    color="muted"
                    showAnatomy={showAnatomy}
                    text={`Tiết kiệm ${formatPrice(original - discounted, currency)}`}
                />
            ) : null}
        </div>
    )
