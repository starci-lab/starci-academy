...
import type { WithClassNames } from "@sb-components/_legacy/designs/rendering/MarkdownContent/local-types"

/**
 * STORYBOOK-LOCAL DESIGN SPEC — faithful port of the src `CodeToHtml`
 * sub-renderer; the `@/components` SnippetIcon is swapped for a sibling-local
 * copy. Synced to `src` later.
 */
...
export interface CodeToHtmlProps extends WithClassNames<undefined> {
     ...
     * card. Set `true` for a RAISED card (`bg-surface shadow-surface`, NO border)
     * when the block sits on the page CANVAS.
     */
    elevated?: boolean
}
 ...
 * (IntersectionObserver). Off-screen blocks show raw code (`<pre>`) until scrolled into view.
