...
interface RagSourceGraphQuestionNodeData extends Record<string, unknown> {
    ...
    question: string
    /** When on, emit `data-anat-part="QuestionCard"` for a `BlockAnatomy` panel to badge. */
    showAnatomy?: boolean
}
...
interface RagSourceGraphSourceNodeData extends Record<string, unknown> {
    ...
    score?: number
    /** When on, emit `data-anat-part="SourceCard"` for a `BlockAnatomy` panel to badge. */
    showAnatomy?: boolean
}
...
const RagSourceGraphQuestionNode = ({ data }: NodeProps) => {
    const { question, showAnatomy } = data as RagSourceGraphQuestionNodeData
    return (
        <>
...
const RagSourceGraphSourceNode = ({ data }: NodeProps) => {
    const { filePath, snippetPreview, score, showAnatomy } = data as RagSourceGraphSourceNodeData
    return (
        <>
            <Handle type="target" position={Position.Left} className="!size-2 !border-none !bg-muted" />
            <div
                data-anat-part={showAnatomy ? "SourceCard" : undefined}
...
export interface RagSourceGraphProps {
    ...
    className?: string
    /** When on, emit `data-anat-part` on this graph's own direct sub-parts (QuestionCard · SourceCard) so a `BlockAnatomy` panel can badge them. */
    showAnatomy?: boolean
}
 ...
 */
export const RagSourceGraph = ({ question, sources, className, showAnatomy = false }: RagSourceGraphProps) => {
    ...
    const { nodes, edges } = useMemo<{ nodes: Array<Node>; edges: Array<Edge> }>(() => {
        ...
        const questionNode: Node = {
            ...
            data: { question, showAnatomy } satisfies RagSourceGraphQuestionNodeData,
        }
