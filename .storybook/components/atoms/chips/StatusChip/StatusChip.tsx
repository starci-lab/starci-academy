...
import { Chip, Skeleton as HeroSkeleton, cn } from "@heroui/react"
...
const StatusChipBase = ({
    ...
    if (isSkeleton) {
        // Leaf skeleton OWNED by the atom (hybrid C) — a pill shimmer matching
        // the chip's own box (HeroUI Chip: h-6 py-0.5 + text-xs leading-5).
        return <HeroSkeleton className={cn("h-6 w-16 rounded-full", className)} data-anat-part="Skeleton" />
    }
    const chip = (
        <Chip
...
/** `StatusChip.*` — generic presentational status chip (tone → HeroUI Chip color). */
export const StatusChip = Object.assign(StatusChipBase, {
    Base: StatusChipBase,
})
