 ...
 * `@/components/blocks/identity/UserAvatar`. Authored in Storybook (not `src`);
 * synced to `src` later. NO `@/components` import — the shared `WithClassNames`
 * base and the `dicebearAvatarUrl` util are inlined locally to keep the port
 * self-contained.
 */
...
/**
 * Local mirror of `@/utils/avatar#dicebearAvatarUrl` — a deterministic DiceBear
 * "thumbs" avatar URL from a seed. Same seed → same face everywhere.
 */
 ...
 */
const UserAvatarBase = ({ username, avatar, seed, size, className, anatPart }: UserAvatarProps) => {
...
/** `UserAvatar.*` — shared resilient user avatar (uploaded → generated → initials). */
export const UserAvatar = Object.assign(UserAvatarBase, {
    Base: UserAvatarBase,
})
