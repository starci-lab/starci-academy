...
import { Feedback, type FeedbackIcon } from "../../feedback/Feedback/Feedback"
...
interface MessageProps {
    ...
    /** Ghi đè icon mặc định của khung. */
    icon?: ReactNode
    /**
...
/**
 * ADAPTER icon NODE → icon COMPONENT.
 *
 * Hợp đồng của HAI khung thông điệp này là `icon?: ReactNode` (caller đưa JSX,
 * kèm cả `weight="duotone"` của phosphor). `Feedback.Empty` — khung nền phía dưới
 * — nhận icon dạng COMPONENT ref và tự ép `size-8` (§4: khung sở hữu scale glyph).
 * Thay vì đổi hợp đồng công khai của `AsyncContent.*` (sẽ bắt mọi caller viết lại
 * icon), khung này BỌC node thành một component nhận `className` rồi thả xuống —
 * `[&>svg]:size-full` để `size-8` của `Feedback.Empty` vẫn tới được svg.
 * ⚠️ Khi `AsyncContent.*` đổi sang icon component ref thì xoá adapter này.
 */
const nodeAsIcon = (node: ReactNode): FeedbackIcon => {
    const IconNode = ({ className }: SVGProps<SVGSVGElement>) => (
        <span className={cn("inline-flex [&>svg]:size-full", className)}>{node}</span>
    )
    return IconNode
}
...
const Empty = (props: AsyncContentEmptyProps) => {
    ...
    return (
        <Feedback.Empty
            ...
            className={className}
            icon={nodeAsIcon(icon ?? <TrayIcon aria-hidden focusable="false" weight="duotone" />)}
            title={title}
...
const ErrorMessage = (props: AsyncContentErrorProps) => {
    ...
    return (
        <Feedback.Empty
            ...
            className={className}
            tone="danger"
            icon={nodeAsIcon(icon ?? <WarningIcon aria-hidden focusable="false" weight="duotone" />)}
            title={title}
            description={description}
            action={composeAction(props)}
        />
