...
interface CheckboxFieldSharedProps {
    ...
    className?: string
    /** When on, emit `data-anat-part` on this field's own direct sub-parts (FieldShell shell · control) so a `BlockAnatomy` panel can badge them. */
    showAnatomy?: boolean
}
...
export const CheckboxField = (props: CheckboxFieldProps) => {
    ...
    const showAnatomy = props.showAnatomy ?? false
                    ...
                    skeletonControl={
                        <div className="flex flex-col gap-2">
                            {Array.from({ length: rows }).map((_, index) => (
                                <Skeleton.Checkbox
                                    key={index}
                                    anatPart={showAnatomy ? "Skeleton.Checkbox" : undefined}
                                />
                            ))}
                        </div>
                    }
                ...
                >
                    ...
                    >
                        {options.map((option) => (
                            <Checkbox
                                key={option.value}
                                data-anat-part={showAnatomy ? "Checkbox" : undefined}
                                value={option.value}
