...
export const FieldShell = ({
    ...
    if (isSkeleton) {
        return (
            <div className={cn("flex flex-col gap-2", className)}>
                ...
                {label != null ? <Skeleton.Typography type="body-sm" width="1/3" anatPart={showAnatomy ? "Label" : undefined} /> : null}
                {/* Control mirror — the input's own control-shaped skeleton, or a field box */}
                <div data-anat-part={showAnatomy ? "Control" : undefined}>{skeletonControl ?? <Skeleton.Input />}</div>
            </div>
        )
    }
    ...
    return (
        <div className={cn("flex flex-col gap-2", className)}>
            ...
            {label != null ? (
                <Label htmlFor={controlId} isDisabled={isDisabled} className="text-sm font-medium" data-anat-part={showAnatomy ? "Label" : undefined}>
                    {label}
                </Label>
            ) : null}
            ...
            {description != null ? (
                <Typography type="body-xs" className="text-muted" data-anat-part={showAnatomy ? "Description" : undefined}>
                    {description}
                </Typography>
            ) : null}

            {/* The live control */}
            <div data-anat-part={showAnatomy ? "Control" : undefined}>{children}</div>

            {/* Error line — danger token, only when invalid */}
            {errorMessage != null ? (
                <Typography type="body-sm" className="text-danger-soft-foreground" data-anat-part={showAnatomy ? "Error" : undefined}>
                    {errorMessage}
                </Typography>
            ) : null}
        </div>
