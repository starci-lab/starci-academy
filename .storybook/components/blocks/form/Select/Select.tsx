...
export interface SelectProps {
    ...
    className?: string
    /**
     * Storybook-only: when true, badges this field's two direct composed parts —
     * `FieldShell` (label/hint/error/skeleton column, opaque here — see its own
     * story for its internals) and `Control` (the live HeroUI Select widget) — via
     * an {@link AnatomyOverlay} marker (neither component accepts `data-anat-part`
     * directly). No visual effect.
     */
    showAnatomy?: boolean
}
...
export const Select = ({
    ...
    // HeroUI Select.Root owns invalid/disabled/open state. Neither it nor FieldShell
    // accepts `data-anat-part` directly, so each is badged via an AnatomyOverlay
    // marker on a `relative` wrapper instead (§11a: only these two DIRECT parts —
    // FieldShell's own label/hint/error stay un-drilled, see its own story).
    const control = (
        ...
        >
            {showAnatomy ? (
                <div className="relative" data-anat>
                    {control}
                    <AnatomyOverlay label="Control" tier="primitive" />
                </div>
            ) : control}
        </FieldShell>
    )
    ...
    return showAnatomy ? (
        <div className="relative" data-anat>
            {fieldShell}
            <AnatomyOverlay label="FieldShell" tier="primitive" />
        </div>
    ) : fieldShell
}
