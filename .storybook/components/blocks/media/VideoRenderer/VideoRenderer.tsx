...
import { type WithClassNames } from "./class-name"

/**
 * STORYBOOK-LOCAL DESIGN SPEC — a faithful port of the
 * `src/components/blocks/media/VideoRenderer` subtree (main resolver + Standard
 * / MpegDash / Youtube players + the VideoControls bar), authored in Storybook
 * (NOT `src`) and synced back to `src` later. No `@/components` imports; the
 * `@/modules` enums / class-name / quality types are inlined as sibling files.
 *
 * NOTE: playback needs live media — Standard/DASH source URLs and the YouTube
 * iframe are all external hosts, so under Storybook's offline/CSP context the
 * frame + control bar render but the media itself does not load.
 */
