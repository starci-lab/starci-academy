            ...
            <Card className={cn(className)}>
                <CardContent className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                        <Skeleton.Chip anatPart={showAnatomy ? "SourceChip" : undefined} />
                        <Skeleton.Typography type="body-sm" width="1/3" anatPart={showAnatomy ? "Title" : undefined} />
                    </div>
                    <Skeleton.Typography type="body-sm" width="3/4" anatPart={showAnatomy ? "Summary" : undefined} />
                </CardContent>
            </Card>
        )
    }
        ...
        <Card className={cn(className)}>
            <CardContent className="flex flex-col gap-2">
                <div className="flex flex-wrap items-center gap-3">
                    <StatusChip tone={SOURCE_TONE_MAP[source] ?? "accent"} anatPart={showAnatomy ? "SourceChip" : undefined}>
                        {sourceLabel}
                    </StatusChip>
                    <Typography type="body-sm" weight="medium" data-anat-part={showAnatomy ? "Title" : undefined}>
                        {title}
                    </Typography>
                    <Typography type="body-xs" color="muted" className="ml-auto" data-anat-part={showAnatomy ? "Date" : undefined}>
                        {date}
                    </Typography>
                </div>
                {courseTitle ? (
                    <Typography type="body-xs" color="muted" data-anat-part={showAnatomy ? "CourseTitle" : undefined}>
                        {courseTitle}
                    </Typography>
                ) : null}
                <Typography type="body-sm" color="muted" data-anat-part={showAnatomy ? "Summary" : undefined}>
                    {summary}
                </Typography>
            </CardContent>
        </Card>
