...
export interface VerdictHeroCardProps {
    ...
    className?: string
    /**
     * Dev/spec: tag this card's own direct parts (`Value` / `Verdict` / `Sub` /
     * `ProgressMeter` / `Splits` / `Action`) so a BlockAnatomy panel can badge them.
     */
    showAnatomy?: boolean
}
...
export const VerdictHeroCard = ({
    ...
    return (
        <SectionCard className={className}>
            ...
            <Typography type="body-sm" weight="semibold" data-anat-part={showAnatomy ? "Verdict" : undefined}>{verdict}</Typography>
            {sub ? (
                <Typography type="body-xs" color="muted" data-anat-part={showAnatomy ? "Sub" : undefined}>{sub}</Typography>
            ) : null}
            ...
            {splits && splits.length > 0 ? (
                ...
                <div className="flex overflow-hidden rounded-2xl border border-default" data-anat-part={showAnatomy ? "Splits" : undefined}>
