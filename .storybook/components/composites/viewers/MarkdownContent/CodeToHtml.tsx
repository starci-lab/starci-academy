    ...
    return (
        ...
        >
            ...
            {html ? (
                ...
                <div
                    data-principle="cell-pad" className="p-3 text-sm [&_code]:!whitespace-pre-wrap [&_code]:!break-words [&_pre]:!whitespace-pre-wrap [&_pre]:!break-words [&_pre]:!bg-transparent [&_pre]:!p-0"
                    dangerouslySetInnerHTML={{ __html: html }}
                />
            ) : (
                /* Fallback: always show raw code while Shiki hasn't finished / on error (avoids an empty box).
                   data-principle hand-set: `Box` has no `"pre"` in its `as` union. */
                <pre data-principle="cell-pad" className="p-3 text-sm whitespace-pre-wrap break-words">
                    <code>{code}</code>
                </pre>
            )}
        </div>
