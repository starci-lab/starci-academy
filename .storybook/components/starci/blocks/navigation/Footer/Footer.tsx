...
const Footer = ({
    ...
    // promoted). `ShowFrom` owns the md visibility switch; wordmark lines stay
    // spans (not banned hosts) with the exact chrome Typography does not own.
    const brandMark = [
        () => <Logo size="footer" />,
        () => (
            <ShowFrom
                                            ...
                                            body={() => (
                                                <StackV
                                                    ...
                                                    items={[
                                                        ...
                                                        () => <StackH gap={3} principle="flex-action" explain="Groups social icon controls on one horizontal peer row so they share a single hit baseline — not chip-row, because these are press targets rather than display tags." items={socialLinkItems} />,
                                                    ]}
                                                />
                                            )}
                                        />
