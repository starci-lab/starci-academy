...
const meta: Meta<typeof TopicLane> = {
    title: "Legacy/Block/Marketing/TopicLane",
    component: TopicLane,
    tags: ["autodocs"],
    parameters: {
        layout: "fullscreen",
...
// sách hàng đổi sang FeedbackEmpty — FeedbackEmpty LÀ một primitive thật (import riêng, có
// anatomy của chính nó) → giữ NGUYÊN 1 node, KHÔNG đào vào con của nó (icon/tiêu đề/
// mô tả rỗng nằm TRONG slot của FeedbackEmpty, không badge riêng).
const EMPTY_PARTS: Array<AnatomyNode> = [
    { name: "Icon", tier: "composite", role: "icon lane — header vẫn giữ" },
    { name: "Typography", tier: "composite", role: "tiêu đề lane (type=body-sm, weight=semibold)" },
    { name: "FeedbackEmpty", tier: "composite", role: "\"Chưa có chủ đề nào\" thay cho danh sách hàng", state: "empty" },
]
...
/** Empty: no topics → the {@link FeedbackEmpty} primitive fills the lane instead of a bare header. */
    ...
    render: () =>
        frame(
            <BlockAnatomy
                ...
                leaf="Empty"
                parts={EMPTY_PARTS}
                note="Không có chủ đề → header giữ nguyên, danh sách hàng đổi sang FeedbackEmpty (khác leaf có dữ liệu)."
            >
                <div className="max-w-[320px]">
            ...
            >
                <div className="flex max-w-[320px] flex-col gap-3">
                    <div className="flex items-center gap-2">
                        <HeroSkeleton className="size-4 shrink-0 rounded" data-anat-part="Skeleton.Icon" />
                        <HeroSkeleton className="my-[5px] h-[14px] w-20 rounded" data-anat-part="Skeleton.Title" />
                    </div>
                    <div className="flex flex-col gap-2">
                        {[0, 1, 2].map((i) => (
                            <div key={i} data-anat-part="Row frame" className="flex items-center justify-between gap-3 rounded-xl border border-default bg-surface px-3 py-2">
                                <HeroSkeleton className="my-[5px] h-[14px] w-1/2 rounded" data-anat-part="Skeleton.Label" />
                                <HeroSkeleton className="h-3 w-6 shrink-0 rounded" data-anat-part="Skeleton.Tag" />
                            </div>
                        ))}
                    </div>
                </div>
