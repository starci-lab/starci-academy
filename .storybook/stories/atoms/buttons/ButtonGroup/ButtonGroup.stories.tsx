import type { Meta, StoryObj } from "@storybook/nextjs"
import { ButtonGroupRoot } from "@sb-components/atoms/buttons/ButtonGroup/ButtonGroup"
import { BlockAnatomy } from "@sb-utils/BlockAnatomy/BlockAnatomy"

const meta: Meta<typeof ButtonGroupRoot> = {
    title: "Atoms/Buttons/ButtonGroupRoot/ButtonGroupRoot",
    component: ButtonGroupRoot,
    tags: ["autodocs"],
    parameters: { layout: "fullscreen" },
}
export default meta
type Story = StoryObj<typeof ButtonGroupRoot>

/** Vendor-boundary atom — chrome only, content handed in. */
export const Default: Story = {
    render: () => (
        <div data-tier="fixture" className="p-8">
            <BlockAnatomy
                name="ButtonGroupRoot"
                tier="atom"
                leaf="Default"
                reason="Smallest house wrap over the vendor primitive so composites never import the library."
                states={[{
                    name: "default",
                    why: "The atom is the vendor boundary. Appearance stays private; callers pass data or named slots.",
                    code: "<ButtonGroupRoot />",
                    render: (<ButtonGroupRoot size="sm"><span data-tier="fixture">A</span></ButtonGroupRoot>),
                }]}
            />
        </div>
    ),
}
