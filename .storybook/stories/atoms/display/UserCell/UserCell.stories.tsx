...
/**
 * ATOM — `UserCell`: hàng người DUY NHẤT của hệ (avatar + tên + `@handle` tuỳ chọn).
 *
 ...
 * (nội dung điền vào mọi leaf, y hệt vai trò của `text` ở `Chip` — không phải một
 * "hình" riêng để test); `avatar` (chỉ đổi ẢNH bên trong `Avatar`, phần đó đã có
 * story riêng của chính nó — `Atoms/Display/Avatar/Avatar`, leaf `Source`; lặp lại
 ...
 * chuyển từ khoá cứng `size-9` sang uỷ quyền `Avatar isSkeleton size={size}` nên
 ...
 *
 * DEPS — `Avatar` và `Typography` đều CÓ story riêng nên khai qua
...
const meta: Meta<typeof UserCell> = {
    title: "Atoms/Display/UserCell/UserCell",
    component: UserCell,
    tags: ["autodocs"],
    parameters: {
        layout: "fullscreen",
...
export const Trailing: Story = {
        ...
        <div className="p-8">
            <BlockAnatomy
                ...
                states={[
                    ...
                    {
                        name: "trailing = <Chip tone=\"accent\" text=\"Admin\" />",
    ...
    handle="@emmafoster"
    trailing={<Chip tone="accent" text="Admin" />}
/>`,
                        render: (
                            <UserCell
                                username="emmafoster"
 ...
 * `isSkeleton` uỷ quyền thẳng `Avatar isSkeleton size={size}` nên vòng tròn đổi
                ...
                states={[
                    {
                        ...
                        why: "The avatar circle mirrors the `sm` preset instead of a hard-coded width, because the shimmer delegates straight to `Avatar isSkeleton size={size}`. The name and (when a handle is set) handle bars mirror the live row's two-line shape at the same density.",
