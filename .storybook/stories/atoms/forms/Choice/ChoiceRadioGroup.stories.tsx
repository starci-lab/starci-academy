...
import { ChoiceRadioGroup } from "@sb-components/atoms/forms/Choice/Choice"
...
/**
 * ATOM — `ChoiceRadioGroup`: a pick-one group, built from `options` DATA (wraps
 ...
 * `annotate`): `ChoiceRadioGroup` CALLS the real `ChoiceRadio` function to build
 ...
 * have this wire — see `ButtonGroup.tsx` passing `anatPart="Button"` down to
 * each `ButtonBase`). So each option row only emits `Control`/`Label` — two
 * INTERNAL slots with the exact same names used in the `ChoiceRadio` story
 * itself — the DOM has no node named `"Radio"`/`"ChoiceRadio"` to anchor a
...
const meta: Meta<typeof ChoiceRadioGroup> = {
    title: "Atoms/Forms/Choice/ChoiceRadioGroup",
        ...
        const Demo = () => {
            ...
            return (
                <BlockAnatomy
                    ...
                    reason="The one radio-group atom in the system, wrapping HeroUI RadioGroup. It rebuilds one ChoiceRadio per entry from options, data, not JSX children, so a caller can never mismatch a row's shape with the rest."
                    states={[
                        {
