...
import { InputTags } from "@sb-components/atoms/forms/Input/Input"
import { BlockAnatomy, type AnatomyAnnotation } from "@sb-utils/BlockAnatomy/BlockAnatomy"

const meta: Meta = { title: "Atoms/Forms/Input/InputTags", tags: ["autodocs"], parameters: { layout: "fullscreen" } }
export default meta
type Story = StoryObj

/**
 * DEP THẬT DUY NHẤT — mỗi token trong hộp là một `Chip` removable (đọc
 * `.storybook/components/atoms/forms/Input/Input.tsx`, `InputTags`: `<Chip
 * onRemove … anatPart="Chip" />` — node name = TÊN COMPONENT THẬT, không
 * còn vai "Chip" của wrapping span cũ, § two-law pass 2026-07-28). `storyId` trỏ
 * export `Removable` của `Atoms/Chips/Chip/Chip` (khớp đúng shape đang render
 * ở đây — có `onRemove`).
 *
...
/** Leaf prop `value` filled — vài token, mỗi token là một `Chip` (dep thật). Migrated to `states` 2026-07-27. */
                    ...
                    states={[
                        {
                            ...
                            why: "Each string in `value` mounts as its own removable `Chip` inside the box, which is the one real dependency this atom composes. Rendering tags as chips instead of plain comma-joined text is what makes each one individually removable.",
