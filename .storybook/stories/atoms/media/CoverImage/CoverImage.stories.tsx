...
/**
 * Chú giải DÙNG CHUNG cho cả ba leaf (IconTile/PricePoint convention, 2026-07-28):
 * chỉ `Skeleton` (HeroUI thật, `tier: "heroui"`) đáng một tên — `WithImage`/`NoImage`
 * render div/img TRẦN, không có sub-part nào là component thật để đặt tên, nên tab
 * Deps của hai leaf đó tự nhiên rỗng (giống `Tile`/`Cover` của `IconTile`). KHÔNG tự
 * khai một key `"CoverImage"` trỏ vào chính root — đó là anti-pattern đã bị gỡ
 * (neo: `Spinner.Base` tự khai part trỏ vào chính nó, thầy bắt 2026-07-26).
 */
