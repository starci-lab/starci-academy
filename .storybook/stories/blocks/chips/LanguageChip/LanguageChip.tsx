import React from "react"
import { DotChip } from "../DotChip/DotChip"

/**
 * STORYBOOK-LOCAL DESIGN SPEC — the target `LanguageChip`. Authored in
 * Storybook (not `src`); synced to `src` later. NO `@/components` imports.
 * The brand-colour / display-name helpers (in `src` they live in
 * `@/modules/utils/language`) are INLINED here so the local port stays
 * self-contained.
 */

/**
 * GitHub-linguist brand colour per language key (lowercase enum value). Language
 * brand colours are an intentional exception to the "tokens only, no hex" rule:
 * they are external brand identity (like a logo colour), not the semantic palette.
 */
const LANGUAGE_COLORS: Record<string, string> = {
    typescript: "#3178c6",
    javascript: "#f1e05a",
    python: "#3572a5",
    java: "#b07219",
    go: "#00add8",
    csharp: "#178600",
    cpp: "#f34b7d",
    c: "#555555",
    rust: "#dea584",
    kotlin: "#a97bff",
    php: "#4f5d95",
    ruby: "#701516",
    swift: "#f05138",
    dart: "#00b4ab",
}

/** Display-name overrides where a plain capitalize is wrong (C#, C++, …). */
const LANGUAGE_LABELS: Record<string, string> = {
    csharp: "C#",
    cpp: "C++",
    c: "C",
    php: "PHP",
}

/** Neutral fallback colour for languages without a brand colour. */
const FALLBACK_COLOR = "#8c95a1"

/** Brand colour for a language key, or a neutral fallback when unknown. */
const getLanguageColor = (key: string): string => LANGUAGE_COLORS[key.toLowerCase()] ?? FALLBACK_COLOR

/** Display name for a language key (proper casing: `csharp`→`C#`, otherwise title-cased). */
const getLanguageLabel = (key: string): string => {
    const lower = key.toLowerCase()
    return LANGUAGE_LABELS[lower] ?? lower.charAt(0).toUpperCase() + lower.slice(1)
}

/** Props for {@link LanguageChip}. */
export interface LanguageChipProps {
    /** Language enum value (e.g. `typescript`, `csharp`). Drives colour + label. */
    language: string
    /** Extra classes on the wrapper. */
    className?: string
    /** Renders a skeleton mirror (dot + label bar) in place of the real chip. */
    isSkeleton?: boolean
}

/**
 * GitHub-style language indicator — a small brand-coloured dot followed by the
 * language's proper display name (`csharp`→C#, `cpp`→C++), like the language tag
 * on a GitHub repo card. A thin map wrapper over the shared {@link DotChip} primitive;
 * colour + label come from the shared {@link getLanguageColor}/{@link getLanguageLabel}.
 *
 * @param props - {@link LanguageChipProps}
 */
export const LanguageChip = ({ language, className, isSkeleton }: LanguageChipProps) => {
    return (
        <DotChip
            dotColor={getLanguageColor(language)}
            label={getLanguageLabel(language)}
            className={className}
            isSkeleton={isSkeleton}
        />
    )
}
