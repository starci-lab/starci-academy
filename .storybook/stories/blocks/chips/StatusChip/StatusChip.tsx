 ...
 * (not `src`); synced to `src` later. NO `@/components` imports — the shared
 * close-× reuses the local {@link ElementCloseButton} port.
 */
...
export interface StatusChipProps {
     ...
     * remove/cancel the chip. Uses the shared {@link ElementCloseButton} with its OWN
     * default × glyph (per `alert.md`: close-affordances use `CloseButton`, never a
     * hand-rolled `<button><XIcon/>` — and don't override its glyph either; same
     * treatment as `Callout`). When set, the leading {@link StatusChipProps.icon} is
     * DROPPED — a removable/filter chip carries EITHER a leading status icon OR a
     * trailing cancel-X, never both (teacher: "có dấu X rồi thì prefix không có icon").
     */
    onCancel?: () => void
    /**
