...
/**
 * ─────────────────────────────────────────────────────────────────────────────
 * STORYBOOK-LOCAL DESIGN SPEC — full port of `@/components/blocks/navigation/TabsCard`.
 * Authored in Storybook (not `src`); synced to `src` later. Uses the sibling
 * local `ExtendedTabs` port (not `@/components`).
 * ─────────────────────────────────────────────────────────────────────────────
 */
 ...
 * use the same secondary chrome, so the row reads as one nav layer — e.g. content
 * tabs on the left and a language switcher on the right. Pass each side as data
 * (items + selectedKey + onSelectionChange); the block owns all styling.
 *
    ...
    const renderGroup = (group: TabsCardGroup, accent = true): ReactNode => (
        ...
        >
            ...
            {/* react-aria's useTab ALWAYS computes an `aria-controls` id pointing at
                ...
                `TabsCard` never shows panel CONTENT here (callers render their own
