...
const LANGUAGE_TABS: Array<ToolbarTabItem> = [
    { key: "vi", label: "Tiếng Việt", icon: <Globe className="size-4" /> },
    { key: "en", label: "English", icon: <Globe className="size-4" /> },
]
...
const addButton: ReactNode = (
    <Button.Icon icon={Plus} ariaLabel="Thêm phần mới" variant="ghost" size="sm" onPress={() => {}} />
)
    ...
    render: () => (
        <div className="p-8">
            <BlockAnatomy
                ...
                code={`<Toolbar.Base
  leftTabs={…}
  leftEnd={<Button.Icon icon={Plus} ariaLabel="Thêm phần mới" variant="ghost" size="sm" />}
/>`}
            >
    ...
    render: () => (
        <div className="p-8">
            <BlockAnatomy
                ...
                leaf="PrimaryVariant"
                parts={LEFT_ONLY_PARTS}
                note="Đổi hình sang segmented-pill full-width; part vẫn CHỈ LeftTabs (không rightTabs/leftEnd)."
                code={"<Toolbar.Base variant=\"primary\" leftTabs={…} />"}
            >
                <Controlled
                    leftItems={[
                        { key: "start", label: "Bắt đầu", icon: <Gear className="size-4" /> },
...
/** `size="sm"` — cùng `variant="primary"` nhưng strip co lại `w-fit`, chữ/padding nhỏ hơn. */
    ...
    render: () => (
        <div className="p-8">
            <BlockAnatomy
                ...
                leaf="Compact"
                parts={LEFT_ONLY_PARTS}
                note={"`size` chỉ tác động lên `variant=\"primary\"` (secondary vốn đã hug-content). Composition không đổi, chỉ đổi tỉ lệ."}
                code={"<Toolbar.Base variant=\"primary\" size=\"sm\" leftTabs={…} />"}
            >
                <Controlled
                    leftItems={[
