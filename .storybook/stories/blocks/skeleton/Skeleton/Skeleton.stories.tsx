...
/** For a raw shimmer placeholder — self-sized via className (h/w/rounded) when you need a loading block that matches no specific component. */
export const Bar: Story = {
    parameters: { usage: "For a raw shimmer placeholder — self-sized via className (h/w/rounded) when you need a loading block that matches no specific component." },
    render: () => (
        <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2">
                <Label>Raw block</Label>
                <Typography type="body-sm" color="muted">
                    Use when you need a shimmer block that matches no component — self-sized via className (h / w / rounded).
                </Typography>
            </div>
...
/**
 * A lookup table for `Skeleton.Typography` by EACH TEXT TIER (type · size): each bar is
 * exactly glyph-height (font-size) and centered in the line-box, so choosing the `type`
 * matching the real text keeps the skeleton from shifting layout when content arrives. With
 * the same font/line-height it applies to both `<Typography type=…>` AND `<div
 * className="text-…">`.
 */
export const TypographySizes: Story = {
    parameters: { usage: "Choose `Skeleton.Typography type=…` matching the real TEXT TIER: the bar is exactly glyph-height (font-size) and centered in the line-box → layout doesn't shift when content arrives. Applies to both `<Typography type=…>` and `<div className=\"text-…\">` (same font/line-height ⇒ same skeleton). Left column skeleton, right is the real Typography of the same type to check the height matches." },
    render: () => (
...
/** A lookup table for picking the right piece when building a loading state: find the kind matching the real node and swap it in. The principle is to mirror the REAL layout tree — keep the structural nodes (separator, wrapper, gap) and only replace the content nodes with Skeleton.<Component>, don't scatter shimmer randomly. If you need a raw block matching no component, use a bare Skeleton (the Bar story) and self-size via className. */
export const AllKinds: Story = {
    parameters: { usage: "A lookup table for picking the right piece when building a loading state: find the kind matching the real node and swap it in. Each row places the skeleton PIECE (left) next to the REAL COMPONENT it mimics (right) so you can eyeball whether the frame matches. The principle is to mirror the REAL layout tree — keep the structural nodes (separator, wrapper, gap) and only replace the content nodes with Skeleton.<Component>, don't scatter shimmer randomly. If you need a raw block matching no component, use a bare Skeleton (the Bar story) and self-size via className." },
    render: () => (
        <div className="flex w-[820px] flex-col gap-8">
...
/** A correctly-composed example: mirror the real layout tree of a user card (avatar + 2 text lines + button), keeping the structure and only replacing content nodes with Skeleton.<Component>. */
export const ComposedExample: Story = {
    parameters: { usage: "A correctly-composed example: mirror the real layout tree of a user card (avatar + 2 text lines + button), keeping the structure and only replacing content nodes with Skeleton.<Component>." },
    render: () => (
        <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2">
                <Label>Mirror the layout</Label>
                <Typography type="body-sm" color="muted">
                    The correct way to compose: mirror the real layout tree (avatar + 2 text lines + button), keep the structure and only replace content nodes with Skeleton.
                </Typography>
            </div>
