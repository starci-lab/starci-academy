 ...
 * thái giờ là `Chip tone=…` và hàng tag là `ChipGroup`, cả hai thuộc tầng
...
interface RowProps {
    /** Compound-member name shown as the row's label, e.g. `ChipEnum`. */
    member: string
    ...
    render: () => (
        <div className="flex flex-col gap-3 p-8">
            {/* `Chip.Status` (2026-07-26) và chip chấm (2026-07-25) đều đã LÊN ATOM, và ở
                đó chúng gộp thành MỘT viên: `Chip tone=…` / `Chip dotClassName=…`.
                Khác tier nên không nằm trong index này — xem `Atoms/Chips/Chip`. */}
            <Row member="ChipEnum">
                <ChipEnum<OrderStatus> value="paid" map={ORDER_STATUS_MAP} />
                <ChipEnum<OrderStatus> value="pending" map={ORDER_STATUS_MAP} />
            </Row>
            <Row member="ChipHighlight">
                <ChipHighlight value={24} label="Module" />
            </Row>
            {/* `Chip.HostPlatform` đã BỎ (2026-07-25): `HostPlatformChip` là tier
                DESIGN (_designs/chips), xem `Design/Chips/HostPlatformChip`. */}
            <Row member="ChipRemovable">
                <ChipRemovable label="Acme Corp" />
            </Row>
            {/* `Chip.Tags` đã thành `ChipGroup` ở tầng ATOM (2026-07-26): hàng tag có
