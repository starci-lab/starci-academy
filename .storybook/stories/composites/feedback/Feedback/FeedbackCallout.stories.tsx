...
/**
 * COMPOSITE (§13) — `FeedbackCallout`: dải tint PHẲNG đặt BÊN TRONG một
 ...
 * `Alert.stories.tsx`): `status` KHÔNG đổi cây DOM — mọi tone dùng chung
 ...
 *
 * DEPS thật: `Action` (khung tự dựng `Button` từ `actionLabel`) và `Close`
 * (khung forward `onClose` xuống atom `Alert`, atom đó tự dựng `Button`
 * cho nút ×) — cả hai đều là component KHÁC có story riêng mà khung dựng lại.
 * `Icon`/`Content`/`Title`/`Description`/`Body` là ruột của atom `Alert`
 * (khung này compose từ nó), KHÔNG phải deps.
 */
const meta: Meta<typeof FeedbackCallout> = {
    title: "Composites/Feedback/Feedback/FeedbackCallout",
...
type Story = StoryObj<typeof FeedbackCallout>

/** Hai node THẬT có story khác để nhảy tới — cả hai đều là `Button` do khung/atom tự dựng. */
                ...
                states={[
                    {
                        ...
                        code: `<FeedbackCallout
  status="default"
  title="Draft saved"
  description="Your changes are kept automatically."
/>`,
                        render: <FeedbackCallout showAnatomy status="default" title="Draft saved" description="Your changes are kept automatically." />,
                    },
                    {
                        name: "status = accent",
...
/>`,
                        render: <FeedbackCallout showAnatomy status="accent" title="Chapter 3 just got a new practice section" description="Reopen the chapter to try what's new." />,
                    },
                    {
                        name: "status = success",
...
/>`,
                        render: <FeedbackCallout showAnatomy status="success" title="Submission successful" description="Results will be ready in a few minutes." />,
                    },
                    {
                        name: "status = warning",
...
/>`,
                        render: <FeedbackCallout showAnatomy status="warning" title="Deadline coming up" description="2 days left to finish this milestone." />,
                    },
                    {
                        name: "status = danger",
...
/>`,
                        render: <FeedbackCallout showAnatomy status="danger" title="Couldn't reach the server" description="Check your connection and try again." />,
                    },
                ]}
            />
        </div>
...
/>`,
                        render: <FeedbackCallout showAnatomy status="accent" title="Tip: highlight a passage to ask AI about it" />,
                    },
                ]}
            />
        </div>
                ...
                states={[
                    {
                        ...
                        code: `<FeedbackCallout status="warning" title="Submission is missing 2 items" description="…">
...
/** `onClose` — dải tự tắt được: thêm node `Close` (atom `Button`) ở cuối hàng. */
