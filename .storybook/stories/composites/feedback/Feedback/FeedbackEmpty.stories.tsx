...
/**
 * KHUNG (composite tier) — `FeedbackEmpty`: chồng dọc CANH GIỮA lấp một chỗ trống
 ...
 * `Alert.stories.tsx`): leaf chỉ tách khi cây DOM thêm/bớt node (`Icon` ·
 ...
 *
 * `icon`/`body`/`action` KHÔNG badge (2026-07-28, §11a.1 LOẠI 3): đó là NODE tuỳ ý
 * caller đưa vào (`icon` đổi hẳn mỗi lần gọi, `body`/`action` là slot tự do) nên
 * không có MỘT component cố định nào để trỏ sang — component ĐÃ bỏ hẳn
 * `data-anat-part` trên ba node này. `Title`/`Description`/`Code` thì NGƯỢC LẠI —
 * chúng luôn là `Typography` (của ta) hoặc HeroUI `Typography` (2 slot heading
 * ở `size="page"`, §9 gap ghi ở cuối file), nên CÓ khai, qua `annotate` (đường hiện
 * hành — `parts` là API cũ, xem `@deprecated` ở `BlockAnatomyProps.parts`).
 */
const meta: Meta<typeof FeedbackEmpty> = {
    title: "Composites/Feedback/Feedback/FeedbackEmpty",
    component: FeedbackEmpty,
    tags: ["autodocs"],
    parameters: {
        layout: "fullscreen",
    },
...
type Story = StoryObj<typeof FeedbackEmpty>
...
export const TitleOnly: Story = {
  ...
  title="No data yet"
/>`,
                        render: <FeedbackEmpty showAnatomy title="No data yet" />,
                    },
                ]}
            />,
        ),
...
/>`,
                        render: <FeedbackEmpty showAnatomy icon={PackageIcon} title="No courses yet" />,
                    },
                ]}
            />,
        ),
