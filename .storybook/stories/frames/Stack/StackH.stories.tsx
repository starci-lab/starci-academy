 ...
 * `divider`. `gap` (§10 scale) and `align` were already demoed in `StackV`,
    ...
    render: () => (
        <div className="p-8">
            <BlockAnatomy
                ...
                tier="frame"
                leaf="Default"
                reason="The same single-axis frame as `StackV`, turned into a row, and only a row gets `wrap`. It accepts ARBITRARY `children`; when the content is N repeating elements of the same kind, that calls for `Cluster`/`Grid` (§13b) instead of this frame."
                states={[
                    {
...
/**
 * Wrap — a state ONLY `StackH` has: when the row runs out of width, children flow
