/**
 * Authoring-convention ESLint rules (2026-08).
 *
 * These close gaps that existing starci-fe rules and audit gates do not cover:
 * - `check-no-vietnamese.mjs` is a Storybook/components gate, not an ESLint rule,
 *   and it does not scan tooling (plugins/eslint, scripts).
 * - `no-emoji.md` is explore/principle prose; nothing in ESLint enforces it in source.
 * - No existing rule requires a named type for destructured parameters.
 *
 * Messages and docs are English-only ([[enforce/authoring/comments]], batch policy).
 */
...
export const CONTENT_ALLOWLIST = [
  ...
  // Storybook anatomy tooling — teacher-sanctioned Vietnamese JSDoc + section marks
  // (ledger: blockanatomy-vi-jsdoc-allowlist-2026-08-07). On-screen copy stays English.
