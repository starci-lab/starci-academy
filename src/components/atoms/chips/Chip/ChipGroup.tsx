...
export const ChipGroup = ({
    ...
    if (isSkeleton) {
        return (
            <div className={cn("flex flex-wrap items-center gap-2", className, classNames)}>
    ...
    return (
        <div className={cn("flex flex-wrap items-center gap-2", className, classNames)}>
            {visibleItems.map(({ key, text }) => (
                <ChipBase key={key} text={text} tone={tone} />
            ))}
            {overflowCount > 0 ? (
                <Tooltip
                    label={
                ...
                >
                    <ChipBase text={`+${overflowCount}`} tone={tone} />
                </Tooltip>
            ) : null}
        </div>
    )
