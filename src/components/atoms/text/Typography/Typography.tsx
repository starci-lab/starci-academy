...
const TypographyBase = ({
    ...
    const bodySize = size as Size
    {
        ...
        if (hasIcons) {
            const iconSpan = (Icon: TypographyIcon, slide: string) => (
            ...
            return (
                ...
                <span className={cn("inline-flex items-center gap-1", iconSlide && "group", baseCls)}>
                    {Prefix ? iconSpan(Prefix, "group-hover:-translate-x-1") : null}
                    <span className={cn("min-w-0", clampCls)}>{renderedText}</span>
                    {Suffix ? iconSpan(Suffix, "group-hover:translate-x-1") : null}
                </span>
            )
        }
        return (
            <span className={cn(baseCls, clampCls)}>
                {renderedText}
            </span>
        )
    }
