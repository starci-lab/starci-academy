/**
 * Bundled latex.js base + article stylesheet (v0.12.6), embedded as a string so the
 * read-only CV preview renders identically to latex.js output without a build-time CSS
 * loader or a runtime network/CSP dependency on node_modules assets. It is injected into a
 * shadow root by {@link LatexCvRenderer}, which isolates these rules from the app chrome.
 * The Computer Modern webfont `@import` is stripped (assets live in node_modules and would
 *404); serif fallback is fine for a preview. Regenerate from node_modules/latex.js/dist/css/
 * (base.css + article.css) if latex.js is upgraded.
 */

/** Raw latex.js base + article CSS (webfont imports stripped), verbatim otherwise. */
export const LATEX_BASE_CSS = `
/*******************/
/* default lengths */
/*******************/
