 ...
 * adding props it silently drops. `Alert` marks its own root as itself, so a
 * `Deps` panel sees "this composite is built from Alert", which is the
 * accurate picture: `Toast` contributes no shape of its own beyond
 * `tone="plain"` + `status` mapping. The `meta` line above is exported anyway
 * so a source-level gate can still resolve this file's tier from the folder
 * path.
 */
