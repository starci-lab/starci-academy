...
/**
 * Loading placeholder for {@link import("../").TopLearners}: mirrors the framed
 * LabeledCard (label + standing header line) + `bordered` `SurfaceListCard`
 * (surface-in-surface: N leader rows — rank · avatar · name · XP · follow) so the
 * block keeps its height when the global leaderboard resolves.
...
export const TopLearnersSkeleton = ({ className }: TopLearnersSkeletonProps) => (
    <div className={cn("flex flex-col gap-3", className)}>
        ...
        {/* the viewer's own standing header line */}
