...
import { TrophyIcon, FlameIcon } from "@phosphor-icons/react"
import React, { useEffect, useMemo, useState } from "react"
import { Accordion, Chip, Typography, cn } from "@heroui/react"
import { DragScrollArea, MarkdownContent, ProgrammingLanguageTabs, ProgrammingLanguageTabsVariant, Score } from "@/components/reuseable"
...
/**
 * SCHEMA V2 challenge VIEW (full-page) — lang-bucket or position-model V2 sections with
 * programming-language tabs. Rendered by {@link import("../ChallengePage").ChallengePage} as its own
 * route (deep-link / back / refresh-safe). Requirements/steps render inner `langs[]` rows (`body`);
 * outputs/prerequisites use `text` only. Grading criteria are intentionally NOT shown — they are an
 * internal rubric, not exposed to the learner.
 *
 * @param props.className - Optional wrapper class (the page sets the available height).
 */
export const ChallengeView = ({ className }: WithClassNames<undefined>) => {
    ...
    return (
        <div className={cn("flex min-h-0 w-full flex-col", className)}>
            <div className="border-b max-w-full w-full py-3">
                <div className="flex w-full flex-col items-center gap-2 px-8">
                    <div className="flex w-full max-w-full justify-center">
                        <Typography type="h4" className="min-w-0 truncate text-center font-bold text-foreground">
                            {statusBadge ? (
            ...
            <div className="mt-0 min-h-0 flex-1 max-w-full overflow-hidden">
                ...
                <div className="flex min-h-0 h-full flex-col lg:grid lg:h-full lg:grid-cols-5">
                    <div className={cn(
                        ...
                        <div className="flex h-full min-h-0 flex-col">
                            <DragScrollArea className="min-h-0 h-full flex-1">
                                ...
                                <div className="p-3">
                                    <Typography type="body" className="font-semibold text-foreground">{t("challenge.yourScore")}</Typography>
                                    <div className="h-3" />
                                    <Score current={earnedScore} max={maxScore} threshold={passThreshold} />
                                    <div className="h-3" />
                                    <Typography type="body-xs" className="text-muted">{t("challenge.minimumPassRequirementAll")}</Typography>
                                </div>
                                ...
                                {prerequisites.length > 0 ? (
                                    <div className="p-3">
                                        <Typography type="body" className="font-semibold text-foreground">{t("challenge.prerequisites")}</Typography>
                                        <div className="h-4" />
                                        <div className="pl-3">
                                ...
                                {requirements.length > 0 ? (
                                    <div className="p-3">
                                        <Typography type="body" className="font-semibold text-foreground">{t("challenge.requirements")}</Typography>
                                        <div className="h-4" />
                                ...
                                {outputs.length > 0 ? (
                                    <div className="p-3">
                                        <Typography type="body" className="font-semibold text-foreground">{t("challenge.outputs")}</Typography>
                                        <div className="h-4" />
                                        <div className="pl-3">
                                ...
                                <div className="border-t border-divider" />
                                <div className="p-3 pb-0">
                                    <Typography type="body" className="font-semibold text-foreground">{t("challenge.submissionModal.title")}</Typography>
                                    <div className="h-2" />
                                </div>
