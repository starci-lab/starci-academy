 ...
 */
const GridBase = ({ items, columns, gap, className, classNames }: GridBaseProps) => (
    <div
        className={cn(
            "grid",
            GAP_CLASS[gap],
    ...
    >
        {items.map((item) => {
            ...
            // A spanning cell needs a real wrapper to hang `col-span-2` on — a `Fragment`
            // cannot carry a class. A plain (non-spanning) cell keeps the old behaviour untouched.
            if (spanClass) {
                return (
