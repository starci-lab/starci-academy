...
export interface UseQueryConsultantSuggestionsSwrOptions {
...
/**
 * Run the `consultantSuggestions` GraphQL query for a prefix and return clean
...
const fetchConsultantSuggestions = async ({
    ...
    const response = await queryConsultantSuggestions({
        request: {
            query,
            limit,
        },
    ...
    const wrapped = response.data?.consultantSuggestions
    if (!wrapped || !wrapped.success) {
        throw new Error(wrapped?.error || wrapped?.message || "Consultant suggestions not found")
    }
...
/**
 * Consultant autocomplete: fetches typeahead suggestions for a (debounced) prefix
 * via the ES Completion Suggester backed `consultantSuggestions` query. Returns
 ...
 */
export const useQueryConsultantSuggestionsSwr = (
    query: string,
    options?: UseQueryConsultantSuggestionsSwrOptions,
) => {
