...
import { queryContentSuggestions } from "@/modules/api/graphql/queries/query-content-suggestions"

/** Cache-key prefix namespacing content suggestions in the SWR cache. */
const SWR_KEY = "QUERY_CONTENT_SUGGESTIONS_SWR"

/** Options forwarded to the generic suggestions factory by this hook. */
export interface UseQueryContentSuggestionsSwrOptions {
...
/**
 * Run the `contentSuggestions` GraphQL query for a prefix and return clean
...
const fetchContentSuggestions = async ({
    ...
    const response = await queryContentSuggestions({
        request: {
            query,
            limit,
        },
    ...
    const wrapped = response.data?.contentSuggestions
    if (!wrapped || !wrapped.success) {
        throw new Error(wrapped?.error || wrapped?.message || "Content suggestions not found")
    }
...
/**
 * Content (lesson) autocomplete: fetches typeahead suggestions for a (debounced)
 * prefix via the ES Completion Suggester backed `contentSuggestions` query. Returns
 * clean `{ id, label }` items ready to render; delegates SWR plumbing (locale-aware
 * key, query gating, keepPreviousData) to {@link useEntitySuggestionsSwr}.
 *
 * @param query - The (debounced) typed prefix.
 * @param options - Optional gate / limit / SWR overrides.
 */
export const useQueryContentSuggestionsSwr = (
    query: string,
    options?: UseQueryContentSuggestionsSwrOptions,
) => {
