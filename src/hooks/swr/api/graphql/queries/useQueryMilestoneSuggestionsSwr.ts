...
import { queryMilestoneSuggestions } from "@/modules/api/graphql/queries/query-milestone-suggestions"

/** Cache-key prefix namespacing milestone suggestions in the SWR cache. */
const SWR_KEY = "QUERY_MILESTONE_SUGGESTIONS_SWR"

/** Options forwarded to the generic suggestions factory by this hook. */
export interface UseQueryMilestoneSuggestionsSwrOptions {
...
/**
 * Run the `milestoneSuggestions` GraphQL query for a prefix and return clean
...
const fetchMilestoneSuggestions = async ({
    ...
    const response = await queryMilestoneSuggestions({
        request: {
            query,
            limit,
        },
    ...
    const wrapped = response.data?.milestoneSuggestions
    if (!wrapped || !wrapped.success) {
        throw new Error(wrapped?.error || wrapped?.message || "Milestone suggestions not found")
    }
...
/**
 * Milestone autocomplete: fetches typeahead suggestions for a (debounced) prefix
 * via the ES Completion Suggester backed `milestoneSuggestions` query. Returns
 ...
 */
export const useQueryMilestoneSuggestionsSwr = (
    query: string,
    options?: UseQueryMilestoneSuggestionsSwrOptions,
) => {
