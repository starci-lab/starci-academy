...
/** Type guard: is a raw query value one of the known DashboardPage tabs? */
const isDashboardTab = (value: string | null): value is DashboardTab =>
    value != null && (DASHBOARD_TABS as ReadonlyArray<string>).includes(value)

/**
 * Two-way bind the shared DashboardPage-tab store to the URL query string, so the open
 ...
 * A `fromUrl` ref breaks the echo loop. Mount once, in the DashboardPage page root.
 */
