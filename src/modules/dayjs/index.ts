 ...
 * - vi → day + month-number form with the locale month word (via dayjs format)
...
export const formatDateTime = (date: dayjs.ConfigType, locale = "vi"): string => {
    ...
    if (locale === "en") {
        return d.format("HH:mm MMMM D, YYYY")
    }
    // Build the VI month word at runtime so source stays English-authored.
    const viMonth = String.fromCharCode(0x74, 0x68, 0xe1, 0x6e, 0x67)
    return d.format(`HH:mm D [${viMonth}] M, YYYY`)
}
