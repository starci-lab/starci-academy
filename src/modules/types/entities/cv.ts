/**
 * Shared FE-owned block model for the CvGalleryPage block editor (`cv_blocks` document).
 *
 ...
 * `CvGalleryPage-BUILDER-BLOCK-EDITOR-BRAINSTORM.md`, "FINALIZED AI + PERSISTENCE": *"FE owns
...
/**
 * One section of the CvGalleryPage document (`cv_blocks.blocks[]` — a JSONB array, order
...
/** CvGalleryPage document language — drives default block titles + AI prompt language. */
export type CvLanguage = "vi" | "en"

/**
 * Which visual TEMPLATE (layout) renders the CvGalleryPage — same block data, different
 ...
 * export — flagged in the gallery). See `CvGalleryPage-TEMPLATES-BRAINSTORM.md`.
 */
...
export interface CvStyle {
    ...
    /** CvGalleryPage language — drives new-block default titles + AI prompt language (default "vi"). */
    language?: CvLanguage
    /** Visual template/layout (default "classic"). */
    template?: CvTemplate
}

/** One CvGalleryPage document (`CvBlocksDocument` — `cv_blocks` row). One user owns many. */
export interface CvDocument {
    ...
    /**
     * The CvGalleryPage's LaTeX (`.tex`) source. Null until the first compile. In block
    ...
    /** Whether this CvGalleryPage is the user's ONE public resume (single-public-per-user). */
    isPublic: boolean
